...
class Cl(ClBase):
    ...
    def __init__(self, pos_sig: int|None=None, neg_sig: int=0, zero_sig: int=0, *,
        ...
        if sig is None:
            ...
            # Build signature
            sig = np.array([0] * zero_sig + [1] * pos_sig + [-1] * neg_sig,
                                dtype=SigType)
        ...
        #
        self._mult_table_res_idx = self._build_mult_table_res_idx()
        self._mult_table = self._build_mult_table(sig)

    def _build_mult_table_res_idx(self) -> NDResultIdxType:
    ...
    def _build_mult_table(self, sig: npt.ArrayLike) -> NDMultTableType:
        """Create multiplication tables"""
        #
        ...
        signature_table = np.where(overlap_mask, sig, 1).prod(axis=-1, dtype=SigType)
        ...
        #
        return sign_table * signature_table
