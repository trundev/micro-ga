...
class Cl(layout.Cl):
    ...
    def from_matrix_ndarray(self, mvect_mtx: npt.NDArray, *,
        ...
        data = np.mean(data, axis=self._expand_axis, where=self._mult_table != 0)
    ...
    def from_vector_matrix(self, mvect_vmtx: npt.NDArray, *, strict: bool=True,
        ...
        if column is None:
            mult_table = self._mult_table
            sl = np.s_[row, :]
        else:
            mult_table = self._mult_table.T
            sl = np.s_[:, column]
        res_idx = self._mult_table_res_idx[sl]
        if (self._expand_axis == -1) ^ (column is None):
            # Vector orientation is along the matrix order,
            # like column-vector from matrix with column based order
            signature = self._mult_table[sl][res_idx]
        else:
            # Vector orientation is orthogonal to the matrix order,
            # like row-vector from matrix with column based order
            signature = mult_table[res_idx, np.arange(mult_table.shape[-1])]
